...
function registerAccessRoutes(app, options = {}) {
    ...
    app.post('/register/verify', async (req, res) => {
        try {
            ...
            if (!checkRateLimit(`verify:${lowerEmail}`, OTP_MAX_ATTEMPTS, 60000) || 
                !checkRateLimit(`verify:${getClientIp(req)}`, OTP_MAX_ATTEMPTS, 60000)) {
    ...
    async function completeVerification(email, req, res) {
        ...
        const deviceIp = pending.deviceIp || getClientIp(req);
...
function createAccessTokenMiddleware(options = {}) {
    ...
    return function requireAccessToken(req, res, next) {
        ...
        if (!token) {
            ...
            return res.status(401).type('html').send(`<!DOCTYPE html><html><body><h1>Access required</h1><p>Complete email verification and use your ${TOKEN_TTL_DISPLAY} access token to continue.</p><p><a href="/register">Open registration</a></p></body></html>`);
        }
